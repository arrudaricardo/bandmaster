...
- [ ] Expected branch and pre-batch commit, clean index state, frozen manifest, recoverable content, ordered commit plan, and journal step are durably recorded before Git mutation.
- [ ] Crash tests terminate finalization at every persisted journal step and resume through a fresh CLI process.
- [ ] A known journaled Git state continues or rolls back to the same all-or-nothing outcome as an uninterrupted failure.
...
- [ ] Provisional commits may remain as unreachable objects after rollback but never remain as a partially accepted batch.
