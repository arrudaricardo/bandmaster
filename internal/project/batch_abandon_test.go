...
func TestMigrateBatchAbandonmentSchemaPreservesForeignKeyTargets(t *testing.T) {
	...
	for _, statement := range []string{
		...
		`CREATE TABLE tasks(batch_id TEXT NOT NULL REFERENCES batches(id))`,
	...
	var target string
	if err := db.QueryRow(`SELECT "table" FROM pragma_foreign_key_list('tasks') LIMIT 1`).Scan(&target); err != nil {
		t.Fatal(err)
	}
	if target != "batches" {
