...
const generatedSkill = "---\n" +
	...
	"---\n\n" +
	"# Bandmaster\n\n" +
	"Use the `bandmaster` CLI for project-local orchestration. Always use JSON output when invoking commands as an agent.\n\n" +
	"Before starting orchestration, run `bandmaster config status --json`. If the current validation configuration is not approved, report its digest and ask the user to review `.bandmaster.yaml` and run:\n\n" +
	"```text\n" +
	"bandmaster config approve <digest> --json\n" +
	"```\n\n" +
	"Never approve validation configuration on the user's behalf. Do not start or resume a Bandmaster session until status reports that the exact current digest is approved.\n"
