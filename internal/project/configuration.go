...
description: Diagnose and explain Bandmaster runtime behavior from sanitized structured evidence. Use when asked to debug, diagnose, inspect, troubleshoot, or explain Bandmaster sessions, agents, tasks, claims, leases, batches, monitors, integrity, configuration, Git state, or orchestration failures.
---
...
3. Interpret stable diagnostic codes, affected identities, evidence, and suggested supported CLI actions. Correlate derived agents with authoritative tasks, leases, and claims; do not invent a persisted Agent entity.
...
type configuration struct {
	...
	AgentLeaseDuration string            `yaml:"agent_lease_duration"`
...
func (p *Project) Initialize(options InitOptions) (InitResult, *Error) {
	...
	if _, err := os.Stat(configPath); errors.Is(err, os.ErrNotExist) {
		...
		config, err := yaml.Marshal(configuration{Version: 2, AgentLeaseDuration: "5m", Validation: validationSection{Commands: commands}})
		if err != nil {
...
func (p *Project) agentLeaseConfiguration() (time.Duration, string, *Error) {
	content, err := os.ReadFile(filepath.Join(p.Root, ".bandmaster.yaml"))
	if err != nil {
		return 0, "", internal("read agent lease configuration", err)
	}
	if projectError := validateConfiguration(content, p.Root); projectError != nil {
	...
	duration, err := time.ParseDuration(config.AgentLeaseDuration)
	if err != nil || duration <= 0 {
		return 0, "", invalid("invalid_configuration", "agent_lease_duration must be a positive Go duration such as 5m.")
	}
	digest := sha256.Sum256(content)
...
func validateConfiguration(content []byte, root string) *Error {
	...
	if err := yaml.Unmarshal(content, &versionProbe); err == nil && versionProbe.Version == 1 {
		return invalid("unsupported_configuration_version", "Bandmaster configuration version 1 uses retired Worker settings. Regenerate with `bandmaster init` after moving the old file aside, review the Agent settings, and approve the new digest.")
	}
	decoder := yaml.NewDecoder(bytes.NewReader(content))
	decoder.KnownFields(true)
	var config configuration
	...
	if duration, err := time.ParseDuration(config.AgentLeaseDuration); err != nil || duration <= 0 {
		return invalid("invalid_configuration", "agent_lease_duration must be a positive Go duration such as 5m.")
	}
