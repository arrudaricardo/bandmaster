...
func commandName(args []string) string {
	...
	case len(args) >= 2 && args[0] == "finalization" && args[1] == "recover":
		return "finalization recover"
	...
	case len(args) == 2 && args[0] == "batch" && (args[1] == "commit" || args[1] == "finalize"):
		return "batch " + args[1]
	case len(args) >= 2 && args[0] == "batch" && args[1] == "abandon":
...
func mutatingCommand(command string) bool {
	switch command {
	case "init", "config approve", "session pause", "session resume", "session finish", "session abort", "integrity recover", "finalization recover", "batch freeze", "batch validate", "batch commit", "batch finalize", "batch abandon",
