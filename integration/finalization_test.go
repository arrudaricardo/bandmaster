...
func TestCommitBatchCreatesOrderedTaskCommitsAndCompletesNoOps(t *testing.T) {
	...
	changedAssignment := successfulTaskCommand(t, repo, "assign", changed.Result.ID, "--agent", "commit-changed")
	noOpAssignment := successfulTaskCommand(t, repo, "assign", noOp.Result.ID, "--agent", "commit-noop")
...
func TestCommitBatchRollsBackHookFailureAndPreservesEdits(t *testing.T) {
	...
	assignment := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--agent", "hook-agent")
...
func TestCommitBatchQuarantinesHookChangeOutsideClaims(t *testing.T) {
	...
	assignment := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--agent", "hook-agent")
	successfulTaskCommand(t, repo, "claim", task.Result.ID, "--token", assignment.Result.AssignmentToken, "--path", "owned.txt")
	writeFile(t, filepath.Join(repo, "owned.txt"), "agent content\n")
	submitBatchTask(t, repo, task.Result.ID, assignment.Result.AssignmentToken)
	successfulBatchCommand(t, repo, "freeze")
	successfulBatchCommand(t, repo, "validate")
	hook := filepath.Join(repo, ".git", "hooks", "pre-commit")
...
func TestFinalizationRecoverRollsBackKnownInterruptedStepsAndIsIdempotent(t *testing.T) {
	for _, step := range []string{"prepared", "committing", "validating"} {
		t.Run(step, func(t *testing.T) {
			...
			assignment := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--agent", "crash-agent")
			successfulTaskCommand(t, repo, "claim", task.Result.ID, "--token", assignment.Result.AssignmentToken, "--path", "owned.txt")
			...
			if response.SchemaVersion != "2" || response.Command != "finalization recover" || !response.Success || response.Result.JournalStep != step || response.Result.Classification != "recognized" || response.Result.Action != "rollback" || response.Result.Outcome != "rolled_back" || !response.Result.Idempotent {
...
func TestFinalizationRecoverDoesNotReplayAnOlderRecoveryForANewInterruption(t *testing.T) {
	...
	assignment := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--agent", "first-agent")
	successfulTaskCommand(t, repo, "claim", task.Result.ID, "--token", assignment.Result.AssignmentToken, "--path", "owned.txt")
	...
	successfulTaskCommand(t, repo, "repair", task.Result.ID,
		"--user-confirmation", "the first agent is stopped",
		"--diagnosis", "the first finalization was interrupted", "--intended-repair", "resubmit the preserved change",
	)
	replacement := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--agent", "second-agent")
...
func TestFinalizationRecoveryAuditFailureLeavesJournalRetryable(t *testing.T) {
	...
	assignment := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--agent", "audit-agent")
	successfulTaskCommand(t, repo, "claim", task.Result.ID, "--token", assignment.Result.AssignmentToken, "--path", "owned.txt")
...
func TestCommitBatchQuarantinesExternalGitStateAfterInterruption(t *testing.T) {
	...
	for name, mutate := range mutations {
		t.Run(name, func(t *testing.T) {
			...
			assignment := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--agent", "external-agent")
			successfulTaskCommand(t, repo, "claim", task.Result.ID, "--token", assignment.Result.AssignmentToken, "--path", "owned.txt")
			writeFile(t, filepath.Join(repo, "owned.txt"), "agent content\n")
			submitBatchTask(t, repo, task.Result.ID, assignment.Result.AssignmentToken)
			successfulBatchCommand(t, repo, "freeze")
