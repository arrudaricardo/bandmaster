...
func TestOrdinaryWorkerFailureRetainsCollectingOwnershipForReplacement(t *testing.T) {
	...
	assignment := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--worker", "worker-failed")
	...
	repairPending := successfulTaskCommand(t, repo, "repair", task.Result.ID,
		"--terminated-worker", "worker-failed",
		"--termination-proof", "codex-handle-worker-failed-stopped",
		"--diagnosis", "worker exited before completing the implementation",
		"--intended-repair", "finish the owned file and validate it",
	)
	if repairPending.Result.Status != "repair_pending" || repairPending.Result.AssignmentToken != "" || repairPending.Result.WorkerIdentity != "" || len(repairPending.Result.Claims) != 1 {
		t.Fatalf("ordinary worker failure did not retain inactive ownership: %+v", repairPending.Result)
	}
	if repairPending.Result.Claims[0].Baseline.ContentHash != claimed.Result.Claims[0].Baseline.ContentHash || readFile(t, filepath.Join(repo, "owned.txt")) != "partial edit\n" {
		t.Fatalf("ordinary worker failure changed the baseline or partial edit: before=%+v after=%+v", claimed.Result.Claims, repairPending.Result.Claims)
	}
	event := repairPending.Result.AuditHistory[len(repairPending.Result.AuditHistory)-1]
	if event.Event != "task_repair_requested" || event.FromStatus != "editing" || event.ToStatus != "repair_pending" || event.WorkerIdentity != "worker-failed" || event.TerminationProof != "codex-handle-worker-failed-stopped" || event.Diagnosis == "" || event.IntendedRepair == "" || len(event.RepairSnapshots) != 1 || event.RepairSnapshots[0].Path != "owned.txt" {
	...
	if batch.Result.Status != "collecting" || len(batch.Result.Members) != 1 || batch.Result.Members[0].TaskID != task.Result.ID {
	...
	replacement := successfulTaskCommand(t, repo, "assign", task.Result.ID, "--worker", "worker-replacement")
...
`)
	...
	leftAssignment := successfulTaskCommand(t, repo, "assign", left.Result.ID, "--worker", "worker-left-original")
	rightAssignment := successfulTaskCommand(t, repo, "assign", right.Result.ID, "--worker", "worker-right-original")
	...
	leftRepair := successfulTaskCommand(t, repo, "repair", left.Result.ID,
		"--terminated-worker", "worker-left-original", "--termination-proof", "left worker stopped after submission",
	...
	if repairing.Result.Status != "repairing" || repairing.Result.BaseCommit != started.Result.StartingCommit || len(repairing.Result.Members) != 2 || len(repairing.Result.Manifest) != 0 {
		t.Fatalf("frozen repair changed base/membership or retained a stale manifest: %+v", repairing.Result)
	}
	if repairing.Result.Members[0].TaskID != left.Result.ID || repairing.Result.Members[1].TaskID != right.Result.ID || repairing.Result.Members[1].Status != "submitted" {
		t.Fatalf("repair changed ownership or reopened an unselected owner: %+v", repairing.Result.Members)
	}

	unrelated := successfulTaskCommand(t, repo, "create", "--title", "Unrelated", "--intent", "Must wait", "--expected-outcome", "Cannot join repair")
	assertTaskError(t, runBandmaster(t, repo, "task", "assign", unrelated.Result.ID, "--worker", "worker-unrelated", "--json"), 2, "batch_repair_in_progress", true)
	...
	rightRepair := successfulTaskCommand(t, repo, "repair", right.Result.ID,
		"--user-confirmation", "I confirmed worker-right-original is no longer running",
	...
	assertBatchError(t, runBandmaster(t, repo, "batch", "freeze", "--json"), 2, "batch_member_not_submitted", true)

	leftReplacement := successfulTaskCommand(t, repo, "assign", left.Result.ID, "--worker", "worker-left-repair")
	rightReplacement := successfulTaskCommand(t, repo, "assign", right.Result.ID, "--worker", "worker-right-repair")
	...
	if refrozen.Result.ID != frozen.Result.ID || refrozen.Result.BaseCommit != frozen.Result.BaseCommit || len(refrozen.Result.Members) != 2 || len(refrozen.Result.Manifest) != 3 {
		t.Fatalf("repair barrier did not preserve and rebuild the original batch: %+v", refrozen.Result)
	}
	for _, member := range refrozen.Result.Members {
		if member.Status != "submitted" {
			t.Fatalf("repair barrier admitted an unsubmitted owner: %+v", refrozen.Result.Members)
		}
	}
